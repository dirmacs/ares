...
| Primitive | File | Rust |
|-----------|------|------|
| **Γ^∞ Context** | `crates/cordis/src/lib.rs` `Context{store,isolate,intercept,fiber,parent,root}` | `Context::new_root()->Arc<Context>`, `extend`, `isolate::<T>(label)`, `intercept::<T>(val)`, `provide::<T:Service>(svc)->Arc<T>` (LIFO undo onto `fiber.acc`), `get::<T:Service>()->Option<Arc<T>>` (intercept→store→parent), `fiber()` |
...
```
Each sub-module `impl Service` + `provide(RouteSet)` via `ctx.plugin`; `routes.rs` becomes `fn build_routes(ctx:&Arc<Context>)->Router`. Same paths/auth (`X-Admin-Secret`) preserved, only file boundaries move. `crates/ares-agent/src/configurable.rs` shows `cfg`→`Service::check()` migration: `struct PostgresService; impl Service for PostgresService { fn check()->bool{cfg!(feature="postgres")} }` and handlers use `if ctx.get::<PostgresService>().is_some()` not `#[cfg]`.
...
```
leaf (zero ARES deps)
  crates/cordis  ─┐  (Context/Fiber/Service/Registry/Events/Loader/Reflect)
                              │
  ...
  crates/ares-memory (merge → ares-agent)            └─► crates/ares-mcp (bridge)
                                                                  │
  crates/ares-agent (execution.rs AgentExecutionService, resolver.rs AgentResolverService, scheduler/pipeline/trigger stubs)
                                                                  ▼
...
YAGNI: `ares-auth` + `ares-memory` merged into `ares-core`/`ares-agent` (decision `docs/cordis-yagni.md`); 9 crates kept.
...
- **Spike correctness:** `cargo test -p cordis` **12 passed** (temporal + spatial + isolate + events + epoch + inertia + registry_single_source + 5 loader round-trip/reconcile)
- **Full verification matrix (Phase 7, step 22):** `cargo check` (both feature sets) **PASS**, `cargo test -p cordis --lib` **12/12**, `cargo test -p ares-tools --lib --features postgres,mcp calculator` **11/11**, `cargo clippy -p cordis, -D warnings` **PASS** (after `ServiceInitFuture` alias fixing `type_complexity`), `cargo test --doc` 0/10 ignored. Full `cargo clippy, -D warnings` still shows baseline `dead_code` (3) + `should_implement_trait` (1), **pre-existing, not new, tracked in `docs/cordis-baseline.md`**, not blocking per ceiling rule.
...
- **AgentExecutionService::execute** dedup skeleton with real `AgentRequest`/`TenantDb`/`ContextProvider`/`ToolCoordinator`/`run_history`/`loop_detector` (12 tests `temporal`/`spatial` + `loader` 5, `cargo test -p cordis` 12/12, `calculator` 11/11).
...
- `cargo miri test` **SKIP**, `miri` component not available for `1.95.0-x86_64-unknown-linux-gnu` (`rustup component add miri` fails on this toolchain, documented per plan: the plan checks leaf crates `ares-types`/`overlay`/`ares-vector`/`ares-memory`, and skips `tokio` crates)
...
- `cargo clippy --no-default-features --features openai,postgres,mcp, -D warnings` **PASS** (0, after `sort_by_key`/`Default` derive/`for_kv_map`/doc allow + `allow(dead_code)` for `ReflectService`, `fallback_chain`, and similar + `allow(unused_imports)` for `ToolConfig` test-only + `allow(explicit_counter_loop)` + `sort_by_key` in `deploy.rs`/`loops.rs`)
...
- **HOLD cleanup:** `db73e24 HMR defer` (`hmr` feature `libloading 0.8` off, `watcher.rs` 9k `watch_many` 500ms debounce → `ReflectService::notify` BFS → `Fiber::refresh`, `hmr.rs` stub `HmrLibrary` RAII, docs `cordis-mapping §10/11` + `remedis`), `da3186e scheduler` (`SchedulerService` 361 lines real tick `tick_ms 60_000`+`db`+`execution`+`_handle` `next_run_at` `cron` crate + `catch-up`/`compute_next`/`skip` as methods, `Service::init` spawns `select! tick+watch`+`Postgres LISTEN` fallback, `src/main.rs` `_root_ctx.provide(SchedulerService::new(..60_000))+ensure_notifier/register_dependent/set_context`+`Service::init`, `TODO cordis 0`), `8b8f61c wiring 8 plugin` (`Cargo.toml` `inventory` default + `cordis/Cargo.toml` `inventory` + `lib.rs` `Context::plugin`+`CordisInventory 8 submits`+`inventory_len`, `src/main.rs` `let root_ctx=Context::new_root()` real not `_root_ctx`, 8×`root_ctx.plugin(ConfigService/CatalogService/ProviderRegistryService/AuthServiceWrapper/AgentServiceWrapper/ToolServiceWrapper/SchedulerService/HealthJobService).await` replaces 17 lets, `build_router(root_ctx.clone())`, `inventory::submit! 8×` Config/Catalog/Provider/Tool/Agent/Auth/Scheduler/Health, `compute_epoch` `Arc` fix, `catalog clone` fix), `AppState HOLD` (per Main OVERRIDE kept `pub struct AppState`+`base_router`+`#![allow(deprecated)]` narrow 3+2+5=11 lines, 177 `State<AppState>` deferred 662 errors, `grep State<AppState` 177 kept), `Decomp HOLD` (admin 3059 kept as shared helpers `#[path]` re-exports, shards 14 real 3530 + v1 1074+3×1233, `handlers/mod.rs` E0761 removed via revert, `grep -R #\[cfg\(feature` handlers **0**)
- `cargo check --no-default-features --features openai,postgres,mcp` **PASS** (0.56s, 6 warnings `never read` wrapper fields + `Permission denied .cargo-ok` only), `cargo check --no-default-features` **PASS** (0.40s), `cargo test -p cordis --lib` **15/15** (was 12/12 +2 watcher +1 hmr), `cargo test -p ares-tools --lib --features postgres,mcp` **193/193** `calc 11`, `cargo test --doc` **1/10**, `cargo miri` **SKIP 1.95** (leaf only), `cargo clippy --no-default-features --features openai,postgres,mcp, -D warnings` **PASS** (0, `io::Error::other` fixed 14, `Permission denied` only), `cargo clippy --no-default-features, -D warnings` **PASS** (0), `cargo clippy -p cordis --features hmr, -D warnings` **PASS** (6.41s), `cargo clippy -p cordis/tools/llm` **PASS** (leaf), `grep -R execute_for_tenant` **0**, `grep -R TODO.*cordis` **0**, `grep allow.*deprecated src/lib.rs` **3** `(#![allow(deprecated)]+2)` intentionally (HOLD), `ls handlers 17 admin 14 v1 4` `admin.rs 3059 v1.rs 1074` (HOLD), `grep CordisInventory` **11** `inventory::submit main` **8** `.plugin(` **8** `root_ctx` real
...
- 1) `curl /health` → 200 OK: `curl -s -w "\nCODE:%{http_code}\n" http://localhost:34784/health` → `OK` `CODE:200`, `curl -s http://localhost:34784/health/detailed` → `{"status":"healthy","version":"0.8.0","checks":{"database":{"status":"healthy"}},"providers":["nvidia"],"agents":["dev_planner","tenant_router",... 23],"latency_ms":0}` `CODE:200`. Same as `main` `0.7.3` `OK` but version `0.8.0` cordis.
- 2) `POST /auth/register` + `POST /auth/login` then `POST /api/chat` with `Bearer` → 200 with `usage` (auth proves, LLM upstream fails same as main): `EMAIL=test-1787312571-34784@dirmacs.com` `curl -s -X POST http://localhost:34784/api/auth/register -H "Content-Type: application/json" -d '{"email":"test-...","password":"Test1234!","name":"Test User"}'` → `{"access_token":"eyJ0eXAiOiJKV1Qi...","refresh_token":"eyJ...","expires_in":900}` `CODE:200` `JWT len 247` `JWT_PREFIX eyJ0eXAiOiJKV1Qi`, `curl -s http://localhost:34784/api/chat -H "Content-Type: application/json" -d '{"message":"hello"}'` without JWT → `{"error":"Unauthorized"}` `CODE:401`, `curl -s -X POST http://localhost:34784/api/chat -H "Authorization: Bearer $JWT" -H "Content-Type: application/jso…`
...
- All `cargo clippy, -D warnings` gates 0 (after `allow` for `missing_docs`/… in `src/lib.rs` + `crates/ares-store/src/lib.rs`). This session adds `watcher.rs` + `hmr.rs` with **0 clippy** (`-D warnings` on `cordis` passes, `cargo clippy -p cordis, -D warnings` and `cargo clippy -p cordis --features hmr, -D warnings` both clean).
