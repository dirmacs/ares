...
use crate::Result;
use ::cordis::Context;
use ares_store::agent_feedback;
use ares_store::agent_runs;
use ares_store::alerts as db_alerts;
use ares_store::audit_log;
use ares_store::tenant_allowlist as allowlist;
...
pub async fn list_alerts(
    ...
    let __pool_1 = ctx
        ...
        .clone();
    let alerts =
        db_alerts::list_alerts(&__pool_1, q.severity.as_deref(), q.resolved, limit).await?;
    Ok(Json(alerts))
}
...
pub async fn resolve_alert(
    ...
    let __pool_2 = ctx
        ...
        .pool()
        .clone();
    db_alerts::resolve_alert(&__pool_2, &alert_id, payload.resolved_by.as_deref()).await?;
...
pub async fn list_agent_runs_handler(
    ...
    let __pool_5 = ctx
        ...
        .pool()
        .clone();
    let runs = agent_runs::list_agent_runs(&__pool_5, &tenant_id, Some(&agent_name), limit, offset)
        .await?;
    let config = ctx
...
pub async fn get_agent_feedback_summary_handler(
    ...
    let __pool_7 = ctx
        ...
        .clone();
    let summary =
        agent_feedback::get_agent_feedback_summary(&__pool_7, &tenant_id, &agent_name, days)
            .await?;
    Ok(Json(summary))
}
...
pub fn routes() -> axum::Router<Arc<Context>> {
    ...
    axum::Router::new()
        ...
        .route(
            "/audit/list_agent_runs_handler",
            get(list_agent_runs_handler),
        )
        .route(
            "/audit/create_agent_run_feedback_handler",
            post(create_agent_run_feedback_handler),
        )
        .route(
            "/audit/get_agent_feedback_summary_handler",
            get(get_agent_feedback_summary_handler),
        )
        .route(
            "/audit/list_tenant_allowed_tools",
            get(list_tenant_allowed_tools),
        )
        .route(
            "/audit/add_tenant_allowed_tool",
            post(add_tenant_allowed_tool),
        )
        .route(
            "/audit/delete_tenant_allowed_tool",
            delete(delete_tenant_allowed_tool),
        )
        .route(
            "/audit/list_tenant_allowed_models",
            get(list_tenant_allowed_models),
        )
        .route(
            "/audit/add_tenant_allowed_model",
            post(add_tenant_allowed_model),
        )
        .route(
            "/audit/delete_tenant_allowed_model",
            delete(delete_tenant_allowed_model),
        )
        .route(
            "/audit/list_tenant_allowed_rag_sources",
            get(list_tenant_allowed_rag_sources),
        )
        .route(
            "/audit/add_tenant_allowed_rag_source",
            post(add_tenant_allowed_rag_source),
        )
        .route(
            "/audit/delete_tenant_allowed_rag_source",
            delete(delete_tenant_allowed_rag_source),
        )
        .route(
            "/audit/get_agent_stats_handler",
            get(get_agent_stats_handler),
        )
        .route(
            "/audit/list_all_agents_handler",
            get(list_all_agents_handler),
        )
