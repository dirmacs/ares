...
mod tests {
    ...
    fn test_tenant_tier_serde_roundtrip() {
        for tier in [
            ...
            assert_eq!(parsed.as_str(), tier.as_str());
        }
    }

    #[test]
    fn test_tenant_tier_partial_eq_and_copy() {
        let a = TenantTier::Pro;
        let b = a;
        assert_eq!(a, b);
        assert_ne!(a, TenantTier::Free);
        assert!(format!("{:?}", a).contains("Pro"));
    }
    ...
    #[test]
    fn test_tenant_quota_default_is_free() {
        let default = TenantQuota::default();
        let free = TenantQuota::free();
        assert_eq!(default.tier, free.tier);
        assert_eq!(default.requests_per_month, free.requests_per_month);
        assert_eq!(default.tokens_per_month, free.tokens_per_month);
    }
    ...
    fn test_tenant_quota_serde_roundtrip() {
        ...
        assert_eq!(parsed.tier, TenantTier::Enterprise);
        assert_eq!(parsed.requests_per_month, u64::MAX);
        assert_eq!(parsed.max_agents, u32::MAX);
    }
    ...
    fn test_tenant_serde_roundtrip_unicode_name() {
        ...
        assert_eq!(parsed.tier, TenantTier::Pro);
    }

    #[test]
    fn test_api_key_serde_roundtrip_with_expiry() {
        let key = ApiKey {
            id: "k2".into(),
            tenant_id: "t1".into(),
            key_hash: "hash".into(),
            key_prefix: "ares_".into(),
            name: String::new(),
            is_active: false,
            created_at: 0,
    ...
    fn test_tenant_context_serde_roundtrip() {
        ...
        assert_eq!(parsed.tier, TenantTier::Dev);
        assert_eq!(parsed.quota.requests_per_day, 2_000);
    }

    #[test]
    fn test_tenant_context_token_overflow_boundary() {
        let ctx = TenantContext::new("t".into(), TenantTier::Free);
        assert!(!ctx.can_use_tokens(u64::MAX, 1));
        assert!(ctx.can_use_tokens(0, 0));
        assert!(!ctx.can_make_request(u64::MAX, 0));
    }

    #[test]
    fn test_enterprise_context_unlimited_requests() {
