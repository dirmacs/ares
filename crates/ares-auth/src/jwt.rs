...
mod tests {
    ...
    fn test_refresh_tokens_are_unique() {
        let service = create_test_service();
        let user_id = "user-refresh-unique";
        ...
        assert_ne!(
            ...
            "refresh tokens should be unique even when generated in the same second"
        );
        assert_ne!(
            service.hash_token(&tokens1.refresh_token),
            service.hash_token(&tokens2.refresh_token),
            "refresh token hashes should differ"
        );

        let claims1 = service
            .verify_token(&tokens1.refresh_token)
            .expect("should verify first refresh token");
        let claims2 = service
            .verify_token(&tokens2.refresh_token)
            .expect("should verify second refresh token");

        assert!(!claims1.jti.is_empty(), "refresh token should include jti");
        assert!(!claims2.jti.is_empty(), "refresh token should include jti");
        assert_ne!(claims1.jti, claims2.jti, "jti values should be unique");
    }

    #[test]
    fn test_claims_deserialize_without_jti() {
        ...
        assert!(claims.jti.is_empty(), "missing jti should default to empty");
    }
    ...
    fn test_jwt_encode_decode_roundtrip() {
        ...
        assert_eq!(access_claims.sub, "roundtrip-user");
        assert_eq!(access_claims.email, "roundtrip@example.com");
        assert_eq!(refresh_claims.sub, "roundtrip-user");
        assert!(!refresh_claims.jti.is_empty());
    }

    #[test]
    fn test_expired_token_rejected_with_zero_leeway() {
        let secret = "test-secret-key-that-is-at-least-32-chars".to_string();
        let service = AuthService::with_leeway(secret.clone(), 900, 604800, 0);
        ...
        let result = service.verify_token_with_leeway(&expired_token, 0);
    ...
    fn test_claim_validation_rejects_tampered_payload() {
        ...
        assert_eq!(segments.len(), 3, "JWT should have three segments");
        segments[1].push('x');
    ...
    fn test_access_token_claims_have_empty_jti() {
        ...
        assert!(claims.jti.is_empty(), "access tokens should not carry jti");
    }
