...
pub fn extract_tenant(ctx: Option<Extension<TenantContext>>) -> Result<TenantContext> {
    ctx.map(|Extension(c)| c)
        .ok_or_else(|| HttpError::from(AppError::Auth("Missing tenant context".to_string())))
}

pub fn set_header(headers: &mut axum::http::HeaderMap, name: &'static str, value: impl ToString) {
...
pub fn normalize_per_page(per_page: Option<u32>, default: u32) -> u32 {
    per_page.unwrap_or(default).min(100)
}

pub fn logs_pagination(page: Option<u32>, per_page: Option<u32>) -> (u32, u32) {
    (page.unwrap_or(1), per_page.unwrap_or(50))
}

pub fn compute_total_pages(total: u64, per_page: u32) -> u32 {
    if per_page == 0 {
        return 0;
    }
...
pub fn paginate_vec<T>(items: Vec<T>, page: u32, per_page: u32) -> Paginated<T> {
...
pub fn list_runs_offset(page: u32, per_page: u32) -> i64 {
    ((page - 1) * per_page) as i64
}

pub fn quota_display_limit(limit: u64) -> Option<u64> {
    if limit == u64::MAX {
        None
    } else {
        Some(limit)
...
pub fn check_tenant_request_quota(
    tc: &TenantContext,
...
pub fn extract_agent_run_message(input: &serde_json::Value) -> String {
    input
        .get("message")
...
pub fn extract_workspace_id(input: &serde_json::Value) -> Option<String> {
    input
        .get("workspace_id")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
...
pub fn format_message_with_context(context: &str, message: &str) -> String {
    format!("{}\n\n---\nUser message: {}", context, message)
}

pub fn llm_token_counts_u64(
    ...
    if let Some(u) = usage {
        ...
        (
            ares_agent::memory::estimate_tokens(input_fallback) as u64,
            ares_agent::memory::estimate_tokens(output_fallback) as u64,
        )
    }
}

pub fn llm_token_counts_u32(
    usage: Option<&ares_llm::client::TokenUsage>,
    input_fallback: &str,
    output_fallback: &str,
) -> (u32, u32) {
...
pub fn execution_metadata_names(
...
pub fn usage_period_start(now: DateTime<Utc>) -> DateTime<Utc> {
    now.date_naive()
        .with_day(1)
        .unwrap()
        .and_hms_opt(0, 0, 0)
