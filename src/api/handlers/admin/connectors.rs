...
pub async fn list_skills(
    ...
    let store = db_skills::SkillStore::new(&ctx.get::<crate::context_services::TenantDbService>().expect("not provided").0.pool().clone());
...
pub async fn run_skill(
    ...
    agent_runs::insert_agent_run_with_id_and_metadata(&ctx.get::<crate::context_services::TenantDbService>().expect("not provided").0.pool().clone(),
        &run_id,
        &tenant_id,
        &agent_name,
        None,
    ...
    .execute(&ctx.get::<crate::context_services::TenantDbService>().expect("not provided").0.pool().clone())
    .await
...
pub async fn list_connectors(
    ...
    let store = db_skills::ConnectorStore::new(&ctx.get::<crate::context_services::TenantDbService>().expect("not provided").0.pool().clone());
...
pub async fn oauth_authorize(
    ...
    let store = ares_db::oauth_credentials::OAuthCredentialStore::new(&ctx.get::<crate::context_services::TenantDbService>().expect("not provided").0.pool().clone());
