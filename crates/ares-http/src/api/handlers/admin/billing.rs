...
pub async fn get_run_cost(
    ...
    let cost = store
        .get_run_cost(&run_id)
        .await?
        .ok_or_else(|| HttpError::from(AppError::NotFound(format!("run cost {run_id} not found"))))?;
    Ok(Json(cost))
}
...
pub async fn get_tenant_budget(
    ...
    let budget = store
        ...
        .await?
        .ok_or_else(|| HttpError::from(AppError::NotFound(format!("tenant budget {tenant_id} not found"))))?;
    Ok(Json(budget))
}
...
pub async fn get_token_budget(
    ...
    let budget = store
        ...
        .await?
        .ok_or_else(|| HttpError::from(AppError::NotFound(format!("token budget {tenant_id} not found"))))?;
    Ok(Json(budget))
}
